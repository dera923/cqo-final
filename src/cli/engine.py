import argparse, yaml, pandas as pd, subprocess, sys
...
def load_spec(p): return yaml.safe_load(open(p,'r',encoding='utf-8'))
...
def cmd_gate(spec):
    ...
    try:
        ...
        raise SystemExit("cqo-viz が見つかりません。external/cqo-viz を導入し、`pip install -e external/cqo-viz` を実行してください。")
...
def main():
    ...
    if args.fn in ["validate"]:  cmd_validate(spec)
    if args.fn in ["gate"]:      cmd_gate(spec)
    if args.fn in ["estimate"]:  cmd_estimate(spec)
    if args.fn in ["selective"]: cmd_selective(spec)
    if args.fn in ["adopt"]:     cmd_adopt(spec)
    if args.fn == "all":
        cmd_validate(spec); cmd_gate(spec); cmd_estimate(spec); cmd_selective(spec); cmd_adopt(spec)
