...
class AGate:
    ...
    def evaluate(
        ...
        # 3. バランスチェック（SMD）
        balance_check = self._check_balance(estimation_result.diagnostics.get("smd", 0.0))
        ...
        if self.strict_mode:
            ...
            # 緩和モード: ERRORレベルのみ判定
            critical_failures = [
                cp for cp in checkpoints if not cp.passed and cp.severity == "ERROR"
            ]
    ...
    def _perform_omega_checks(self, additional_checks: Dict[str, Any]) -> List[GateCheckpoint]:
        ...
        if "invariant_score" in additional_checks:
            inv_check = GateCheckpoint(
                name="invariance",
                passed=additional_checks["invariant_score"]
                >= self.thresholds["invariant_score_min"],
    ...
    def _generate_recommendations(
        self, checkpoints: List[GateCheckpoint], violations: List[ViolationType]
    ) -> List[str]:
        """改善推奨事項の生成"""
        recommendations = []
        ...
        if ViolationType.OVERLAP in violations:
            recommendations.append(
                "Consider trimming extreme propensity scores or using overlap weights"
            )
    ...
    def _generate_signature(self, checkpoints: List[GateCheckpoint], timestamp: datetime) -> str:
        """監査署名の生成（改竄防止）"""
        content = {
            "timestamp": timestamp.isoformat(),
            "checkpoints": [
                {"name": cp.name, "passed": cp.passed, "value": cp.value} for cp in checkpoints
            ],
        }
