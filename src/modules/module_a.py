...
class EstimationResult:
    ...
    def __post_init__(self):
        """NASA標準: 事後条件の検証"""
        assert (
            self.ate_ci_lower <= self.ate <= self.ate_ci_upper
...
class DoublyRobustEstimator:
    ...
    def __init__(
        ...
        self.propensity_model = propensity_model or lgb.LGBMClassifier(
            n_estimators=100, max_depth=5, learning_rate=0.1, random_state=random_state, verbose=-1
        )
    ...
    def _fit_nuisance_models(
        self, X_train: np.ndarray, T_train: np.ndarray, Y_train: np.ndarray, fold_idx: int
    ) -> Tuple[BaseEstimator, Dict[int, BaseEstimator]]:
        """
    ...
    def _calculate_influence_function(
        self, Y: np.ndarray, T: np.ndarray, e: np.ndarray, mu0: np.ndarray, mu1: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def fit(self, X: np.ndarray, T: np.ndarray, Y: np.ndarray) -> EstimationResult:
        ...
        try:
            ...
            # 診断統計量の計算
            diagnostics = self._compute_diagnostics(propensity_scores_all, T, Y, mu0_all, mu1_all)
            ...
            logger.info(
                f"Estimation complete: ATE={ate:.4f} [{ate_ci_lower:.4f}, {ate_ci_upper:.4f}]"
            )
    ...
    def _compute_diagnostics(
        self, e: np.ndarray, T: np.ndarray, Y: np.ndarray, mu0: np.ndarray, mu1: np.ndarray
    ) -> Dict[str, Any]:
        """診断統計量の計算"""
