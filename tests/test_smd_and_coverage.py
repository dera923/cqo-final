...
def test_smd_guard_respects_ssot():
    ...
    if {"feature", "smd"}.issubset(set(lowcols)):
        ...
        if not present:
            pytest.skip(
                f"SMD features {SMD_FEATS} not present; available={list(smd.index)[:10]}"
            )
        for f in present:
        ...
        for f in present:
            assert (
                df[f].astype(float).abs().max() <= SMD_MAX + 1e-12
            ), f"SMD({f}) max>{SMD_MAX}"
...
def test_adopt_row_has_positive_lcb95():
    ...
    flag = (
        ...
        else ("adopted" if "adopted" in df.columns else None)
    )
