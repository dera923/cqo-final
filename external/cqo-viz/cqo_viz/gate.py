...
def gate_report_main(csv_path: str, eps: float, alpha: float,
                     w95_max: float, w99_max: float, smd_max: float,
                     features: str):
    ...
    metrics = dict(epsilon_star=eps_star, adopted_epsilon=eps_adopt,
                   w95=w95, w99=w99, tail_prob=tail, max_SMD=max_smd, overall_pass=ok)
    # CSV 追記＆PNG
    tab_dir = Path("reports/tables"); fig_dir = Path("reports/figures")
    tab_dir.mkdir(parents=True, exist_ok=True); fig_dir.mkdir(parents=True, exist_ok=True)
    row = pd.DataFrame([metrics])
    sm_path = tab_dir/"summary_metrics.csv"
    if sm_path.exists(): row.to_csv(sm_path, mode='a', index=False, header=False)
    else:               row.to_csv(sm_path, index=False)
